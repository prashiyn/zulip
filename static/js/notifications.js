...
if (window.webkitNotifications) {
    ...
    notifications_api = {
        ...
        createNotification: function createNotification (icon, title, content, tag) {
            const notification_object = new window.Notification(title, {
                icon: icon,
                body: content,
                tag: tag
            })
            notification_object.show = function () {}
            notification_object.cancel = function () {
...
if (window.electron_bridge !== undefined) {
    ...
    window.electron_bridge.on_event('send_notification_reply_message', function (
        message_id,
        reply
    ) {
...
function process_notification (notification) {
    ...
    if (notification.desktop_notify && /webkit/i.test(navigator.userAgent)) {
        ...
        notification_object = notifications_api.createNotification(
            icon_url,
            title,
            content,
            message.id
...
exports.register_click_handlers = function () {
    $('#out-of-view-notification').on('click', '.compose_notification_narrow_by_topic', function (
        e
    ) {
    ...
    $('#out-of-view-notification').on('click', '.compose_notification_scroll_to_message', function (
        e
    ) {
        const message_id = $(e.currentTarget).data('message-id')
